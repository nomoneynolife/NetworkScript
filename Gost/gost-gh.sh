...
function Install_ct()
{
    ...
    `wget --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/nomoneynolife/NetworkScript/main/Gost/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system`
    `mkdir /etc/gost && wget --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/nomoneynolife/NetworkScript/main/Gost/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost`
...
function read_d_ip()
{
    ...
    echo -e "    具体IP地址的填写, 取决于接收该流量的服务正在监听的IP(详见: https://ghproxy.com/https://github.com/nomoneynolife/EasyGost)"
    read -p "请输入: " flag_c
}
